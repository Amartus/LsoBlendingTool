...
public class ConvertOneOfToAllOffInheritence implements Consumer<OpenAPI> {
    ...
    private static final List<String> discriminatorCandidates = List.of(
            "@type",
            "mapType"
    );
    ...
    private void process(Map.Entry<String, Set<String>> toResolve) {
        ...
        var disc = findDiscriminator(schemasToResolve);
        if (disc.isEmpty()) {
    ...
    private Optional<String> findDiscriminator(Set<Map.Entry<String, Schema>> schemas) {
        return discriminatorCandidates.stream()
                .filter(k -> {
                    var hasProperty = toPredicate(k);
                    return schemas.stream().allMatch(e -> hasProperty.test(e.getValue()));
                }).findFirst();
    }

    private Predicate<Schema> toPredicate(String keyword) {
