...
class ProductSpecReaderTest {
    ...
    public void testReaderForSchema() {
        ...
        var schemas = new ProductSpecReader("testToAugment", dirPath, "model-js.json")
                .readSchemas();
    ...
    public void testReaderForSchemaOas() {
        var dirPath = Utils.toPath("mini-model");
        var schemas = new ProductSpecReader("testToAugment", dirPath, "model-oas.yaml#/components/schemas/ModelOAS")
                .readSchemas();
        assertEquals(7, schemas.size());
    }
}